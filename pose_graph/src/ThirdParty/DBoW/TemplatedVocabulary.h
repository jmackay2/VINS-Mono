...
void TemplatedVocabulary<TDescriptor,F>::loadBin(const std::string &filename) {
  ...
  for(size_t i = 0; i < static_cast<size_t>(voc.nNodes); ++i)
  {
  ...
  for(size_t i = 0; i < static_cast<size_t>(voc.nWords); ++i)
  {
    NodeId wid = static_cast<int>(voc.words[i].wordId);
    NodeId nid = static_cast<int>(voc.words[i].nodeId);
